...
commands = {
    ...
    "introduce yourself": lambda: paralell_introduce(),
...
def paralell_introduce():
    ...
    root.after(0, introduce_showing_gif)
...
def start_showing_gif():
    global gif_player_introduce  # Объявляем gif_player как глобальную переменную
    gif_player_introduce.showgifintroduce('speak', duration=13000)

async def showgifintroduce():
    global gif_player_introduce  # Объявляем gif_player как глобальную переменную
    play_greeting_music()

    # Запускаем основной цикл Tkinter
    gif_player_introduce = tk.Label(root, bg='#000000')
    gif_player_introduce.pack(fill=tk.BOTH, expand=True)

    gif_player_introduce = ShowingGif(gif_player_introduce, size=(200, 344))
...
def introduce_showing_gif():
    ...
    gif_player.show_gif('introduce', duration=20000)
