...
def intro_showing_gif():
    global gif_player_intro  # Объявляем gif_player как глобальную переменную
    gif_player_intro.show_gif(duration=20000)



async def introgif():
    global gif_player_intro
    play_greeting_music()
    ...
    gif_player_intro = ShowingGif(gif_lb, size=(200, 344))

    # Используем root.after() для запуска показ GIF
    root.after(0, intro_showing_gif)
    root.mainloop()
